...
Discover.prototype.reconcile = function () {
  ...
  async.parallel({
    ...
    if (results.publishedContainers) {
      results.publishedContainers.forEach(function (item) {
...
Discover.prototype.unpublish = function unpublish(containerId, cb) {
  ...
  self.etcd.get(containerPath, function (err, result) {
    ...
    var published;
    try {
      published = JSON.parse(result.value);
    } catch (e) {
    ...
    try {
      services.forEach(function (service) {
        var servicePath = published[service],
            errors = [];
        self.etcd.del(servicePath, function (err) {
          if (err) {
    ...
    self.etcd.del(containerPath, function (err) {
      if (err) {
